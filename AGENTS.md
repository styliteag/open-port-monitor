...
- **Resolution order** inside the GVM, nuclei, and NSE alert generators (`app/services/vulnerability_results.py`, `app/services/nse_results.py`): network rule → global rule → native severity. The resolved severity is what the network's alert threshold is applied to (`gvm_alert_severity`, `nuclei_severity`, or the NSE default of `medium`) and is stored on the resulting `Alert.severity_override`. Alert messages include a `(severity promoted from X to Y via rule)` note when a rule fired. NSE findings additionally require a CVE or `VULNERABLE` marker to alert on their native severity — keyword-inferred `WEAK`/`DEPRECATED` findings only alert when a severity rule is in place.
