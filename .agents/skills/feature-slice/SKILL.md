...
Build the feature bottom-up in the order below. Each step has a gate; do not move on until the gate passes. The result is ONE commit spanning every layer (see AGENTS.md §2 — never split backend and frontend commits).
...
- Follow the extension patterns in AGENTS.md §5 (ScannerProtocol / post-phase / own job path).
...
- One commit, conventional format, detailed body (why + scoping + how verified). Run `/ship` if available, otherwise follow AGENTS.md §7 "Commit" checklist.
