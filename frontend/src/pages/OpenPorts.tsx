...
const OpenPorts = () => {
  ...
  useEffect(() => {
    setOffset(0)
  }, [portRangeInput, ipRangeInput, searchTerm, sortKey, sortDirection])
  useEffect(() => {
    if (!toast) return
    ...
    <div className="p-8 max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex gap-4">
          <button
            ...
            className="px-6 py-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-white border-2 border-slate-100 dark:border-slate-800 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:border-indigo-100 transition-all shadow-sm active:scale-95"
          >
            Export Metadata
          </button>
          <Link
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800/50 shadow-sm group hover:shadow-xl transition-all">
          ...
          <p className="text-5xl font-black mt-2 text-indigo-600 tracking-tighter">
            {ports.length}
          </p>
        </div>
        ...
        <div className="md:col-span-2 bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden group">
          ...
          <input
            ...
            placeholder="Search IP, service, mac..."
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.05)] overflow-hidden">
        <div className="px-10 py-8 bg-slate-50/30 dark:bg-slate-800/20 flex flex-wrap gap-10 border-b border-slate-50 dark:border-slate-800/50">
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              onChange={(e) => setPortRangeInput(e.target.value)}
              className="bg-white dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800 rounded-2xl px-5 py-3 text-xs font-black focus:ring-8 ring-indigo-500/5 focus:border-indigo-500/30 outline-none w-44 transition-all shadow-inner uppercase tracking-widest"
              placeholder="80-443"
            />
          </div>
          <div className="flex flex-col gap-2">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-50 dark:border-slate-800/50 bg-white dark:bg-slate-900">
                ...
                <th className="px-10 py-6 text-right text-[11px] font-black text-slate-400 uppercase tracking-widest">
                  Action
                </th>
              </tr>
            </thead>
                ...
                return (
                  <React.Fragment key={k}>
                    <tr className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            onClick={() => {
                              ...
                              setToast({ message: 'Address Copied', tone: 'success' })
                            }}
                            className="opacity-0 group-hover:opacity-100 p-2 bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 rounded-xl transition-all hover:scale-110 active:scale-95"
                          >
                            <svg
                              className="w-4 h-4"
                              fill="none"
                      ...
                      <td className="px-10 py-8 text-slate-700 dark:text-slate-300">
                        <span className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-xl font-black text-sm">
                          {p.port}
                        </span>
                      </td>
                      <td className="px-10 py-8">
                        <span
                          className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest ${p.protocol === 'tcp' ? 'bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300' : 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300'}`}
                        >
                          {p.protocol}
                        </span>
                      </td>
                      <td className="px-10 py-8 text-slate-500 dark:text-slate-400 text-sm font-bold italic tracking-tight">
                        {p.service_guess || 'unidentified'}
                      </td>
                      <td className="px-10 py-8 text-right">
                        <div className="flex justify-end items-center gap-4">
                          {allowed ? (
                            <button
                              onClick={() => handleRemoveWhitelist(p)}
                              className="px-5 py-2 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[11px] font-black uppercase tracking-[0.2em] rounded-2xl border-2 border-emerald-500/10 hover:bg-emerald-500/20 transition-all"
                            >
                              Cleared
                            </button>
                          ) : (
                            isAdmin && (
                              <button
                                onClick={() => setWhitelistTarget(p)}
                                className="px-5 py-2 bg-slate-50 dark:bg-slate-950 text-slate-400 text-[11px] font-black uppercase tracking-[0.2em] rounded-2xl border-2 border-slate-100 dark:border-slate-800 hover:text-indigo-600 hover:border-indigo-500/30 transition-all"
                              >
                                Authorize
                              </button>
                            )
                          ...
                          <button
                            ...
                            className={`p-3 rounded-2xl transition-all ${isOpen ? 'bg-indigo-500 text-white shadow-lg' : 'text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                          >
                            <svg
                              className={`w-6 h-6 transition-transform duration-500 ${isOpen ? 'rotate-180 scale-110' : ''}`}
                              fill="none"
                              viewBox="0 0 24 24"
                              stroke="currentColor"
                            >
                        ...
                        <td colSpan={5} className="px-16 py-12">
                          <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
                            <div className="space-y-8">
                              <div>
                                <h4 className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.25em] mb-4">
                                  Infrastructural Footprint ({p.seen_by_networks.length})
                                </h4>
                                <div className="flex flex-wrap gap-2.5">
                                  {p.seen_by_networks.map((id: number) => (
                                    <span
                                      key={id}
                                      className="px-4 py-2 bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl text-[11px] font-black text-slate-600 dark:text-slate-300 shadow-sm"
                                    >
                                      {networkMap.get(id) || `Gateway ${id}`}
                                    </span>
                                  ))}
                                </div>
                              </div>
                              <div className="grid grid-cols-2 gap-8 pt-4 border-t border-slate-100 dark:border-slate-800/50">
                                <div>
                                  ...
                                  <p className="text-xs font-black text-slate-700 dark:text-slate-300">
                                    {formatDateTime(parseUtcDate(p.first_seen_at))}
                                  </p>
                                </div>
                                <div>
                                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">
                                  ...
                                  <p className="text-xs font-black text-slate-700 dark:text-slate-300">
                                    {formatDateTime(parseUtcDate(p.last_seen_at))}
                                  </p>
                                </div>
                              </div>
                            </div>
                            <div className="space-y-8 border-l border-slate-100 dark:border-slate-800/50 pl-16">
                              ...
                              <div>
                                ...
                                <p className="text-sm font-black text-indigo-600 uppercase tracking-widest italic">
                                  {p.mac_vendor || 'Proprietary Hardware'}
                                </p>
                              </div>
                            </div>
        ...
        <div className="p-8 bg-slate-50/50 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-800/50 flex justify-between items-center px-12">
          <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em]">
            Telemetry sync active • {ports.length} Rows
          </span>
          <div className="flex gap-4">
            <button
              disabled={offset === 0}
              onClick={() => setOffset(Math.max(0, offset - limit))}
              className="px-8 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 disabled:opacity-30 hover:border-indigo-100 transition-all shadow-sm active:scale-95"
            >
              Previous
            </button>
            <button
              disabled={ports.length < limit}
              onClick={() => setOffset(offset + limit)}
              className="px-10 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest bg-slate-900 text-white dark:bg-white dark:text-slate-900 disabled:opacity-30 hover:scale-[1.03] active:scale-95 transition-all shadow-2xl"
            >
              Next Batch
            </button>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-16 rounded-[4rem] w-full max-w-2xl border border-slate-100 dark:border-slate-800 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] relative overflow-hidden animate-in zoom-in-95 duration-500">
            ...
            <div className="relative">
              ...
              <p className="text-slate-500 mt-6 text-lg leading-relaxed font-bold">
                Granting universal authorization for{' '}
                <span className="text-indigo-600 underline underline-offset-8 decoration-4 decoration-indigo-100 dark:decoration-indigo-900/50">
                  {whitelistTarget.port}/{whitelistTarget.protocol}
                </span>{' '}
                on
                <span className="text-indigo-600 block mt-2 text-3xl tracking-tighter">
                  {whitelistTarget.ip}
                </span>
              </p>
              ...
              <form onSubmit={handleWhitelistSubmit} className="mt-12 space-y-10">
                <div className="space-y-4">
                  ...
                  <input
                    ...
                    placeholder="e.g. Authorized customer API gateway exposure"
                    className="w-full border-4 border-slate-50 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 rounded-[2rem] px-8 py-6 text-lg font-black focus:ring-[16px] ring-indigo-500/5 focus:border-indigo-500 focus:bg-white outline-none transition-all shadow-inner tracking-tight"
                  />
                </div>
                ...
                <div className="flex items-center gap-8 pt-6">
                  <button
                    type="button"
                    onClick={() => setWhitelistTarget(null)}
                    className="text-[11px] font-black text-slate-400 hover:text-slate-900 dark:hover:text-white uppercase tracking-[0.3em] transition-all px-4"
                  >
                    Cancel
                  </button>
                  <button
