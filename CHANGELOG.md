...
- **Frontend**: the `/settings/security` page is now "Account Security" and bundles both self-service credentials concerns in one place: a new password change card (current/new/confirm with inline validation, signs out and redirects to login after success because all sessions are invalidated) above the existing 2FA enrollment/management card. Reached as before via the header email badge.
- **Backend**: UI v3 alert list API extension (alert-state-action-matrix decision). `GET /api/alerts/` accepts `queue_state=inbox|out_of_inbox` (maps onto `dismissed`; conflicting combination returns 400) and `policy_state=none|allowed|allowed_network|allowed_global`. Policy state is derived in the new `services/alert_policy.py` by matching enabled `accepted` alert rules (network rules win over global, same resolution order as severity rules); because rule criteria are JSON port ranges, the policy filter is evaluated in Python over the full candidate set before pagination so totals and page boundaries stay correct. Every list response row now carries `queue_state`, `policy_state`, `allow_rule_id`, and `allow_scope`. No schema changes; 12 new tests in `test_alert_policy_states.py`.
- **Frontend**: UI v3 Phase 1 — the Alerts page is now a keyboard-driven triage inbox (ADR 0003): split view with the alert list left (status presets Open/Muted/Allowed/All backed by the new server-side queue/policy filters, search, source and network filters, severity dot counts, pagination) and the full alert detail right (extracted into `AlertDetailPanel`, reused by the deep-linkable `/alerts/$alertId` route). Shortcuts: `j`/`k` navigate, `A` opens the Allow dialog (scope explicit, defaults to the alert's network, reason required), `M` opens the Mute dialog, `S` opens the assign control, `Enter` opens the host — shortcuts only open dialogs, never mutate directly, and are suppressed in inputs/dialogs/menus. Action wording is now effect-based throughout (Allow = permanent rule, never alerts again; Mute = hidden until the next scan re-raises). The dashboard is rebuilt as a triage dashboard: open-alert counts by severity (clickable into the inbox), scanner health, 30-day trend, latest and upcoming scans, and an "Open Inbox" CTA.
- **Frontend**: UI v3 Phase 2 — network creation is now a guided wizard (ADR 0004): Basics → Scan type → Schedule → Alerting → Review, with per-step validation and a review summary before submit; cloning pre-fills the wizard. Editing an existing network uses direct-access tabs with the same four groups instead of a step sequence. The former 806-line `NetworkForm` monolith was split: shared form state, defaults, and the payload mapping (minutes→seconds conversion, Greenbone/nuclei field nulling, `alert_config` merge preserving unmanaged keys) moved to `features/networks/components/form/useNetworkForm.ts` (now unit-tested, 12 new tests), and the visual sections became reusable components (`BasicsFields`, `ScanFields`, `ScheduleFields`, `AlertingFields`). Invalid combinations remain impossible to select (Greenbone hides phases/NSE/nuclei and shows the GVM block; nuclei auto-disables on the switch to Greenbone). Initial port rules are intentionally not part of the wizard — they stay on the network detail page (parity-matrix decision).
- **Frontend**: UI v3 Phase 3 — Hosts & Scans behavior inventory recorded in the parity matrix (hosts list/detail kept unchanged — the detail page already matches the v3 tab pattern). The scan list gains a network column and a visible network filter (the `network_id` search param existed but was never rendered as a control); network names resolve client-side from the cached networks list. The standalone `/nse/results` page is retired from all entry points: scan detail and host detail already show all NSE/GVM/nuclei findings via the per-scan vulnerabilities endpoint, and the alert detail "Vulnerability Context" link now opens the affected host's findings instead of the NSE results page. The route itself stays URL-reachable until the Phase 6 cut-over; the cross-scan CVE search is recorded as an approved Drop in the parity matrix, to be revisited if the beta misses it.
- **Frontend**: UI v3 Phase 4 — the scattered configuration pages are bundled into two hubs (ADR 0002). **Scan Templates** (`/scan-templates`) combines NSE profiles, the NSE script library (with editor deep link), and the GVM library as tabs; **Alerting** (`/alerting`) combines alert rules, severity rules, and SSH alert defaults. The six former standalone pages were extracted unchanged into feature components (`NseProfilesPage`, `NseLibraryPage`, `GvmLibraryPage`, `AlertRulesPage`, `SeverityRulesPage`, `SshAlertDefaultsPage`); the old routes keep rendering them until the Phase 6 cut-over, and the hub tabs are deep-linkable via `?tab=`. Scanners stay unchanged (parity-matrix Keep).
- **Backend**: UI v3 executive overview endpoint `GET /api/overview/executive` (ADR 0005, readable by every role): per-network traffic-light status (red = open critical alerts, amber = open high, green otherwise, using the shared lightweight severity mapping incl. overrides, now in `services/alert_severity.py`), the top 5 open risks (severity rank, newest first), the open-alert total, and a handled-in-30-days count (approximated via `created_at` of dismissed alerts — there is no `dismissed_at` column). New `services/overview.py` + `routers/overview.py` + `schemas/overview.py`; 6 tests in `test_overview.py`.
- **Frontend**: UI v3 Phase 5 — the Executive Overview page (`/overview`) replaces its placeholder: traffic-light list per network, 30-day risk trend (reusing the alert-trend module), top open risks linking into the inbox, and open/handled totals — read-only, management-friendly. Administration is consolidated: `/admin/users` gains a Roles tab (read-only permissions reference, deep-linkable via `?tab=roles`), and `/admin/system` hosts the hostname cache (page body extracted to `HostnameLookupPage`, old route keeps working until cut-over). Account security was already reachable from the header's email badge, recorded as done in the parity matrix.
- **Frontend**: UI v3 Phase 6 (cut-over) — every retired v2 route now permanently redirects to its new home (`/trends`→`/`, `/nse/profiles|library`→`/scan-templates`, `/nse/results`→`/scans`, `/admin/gvm-library`→`/scan-templates?tab=gvm`, `/alert-rules`→`/alerting`, `/admin/severity-rules|ssh-alert-defaults`→`/alerting?tab=…`, `/admin/roles`→`/admin/users?tab=roles`, `/admin/hostname-lookup`→`/admin/system`), so v2 bookmarks keep working. The parity audit closed the two remaining inbox gaps: row checkboxes with a bulk-action bar (Allow with explicit scope, Mute, Delete with severity-breakdown confirmation — existing bulk endpoints) and a CSV/PDF export menu that maps the status preset onto the export endpoint's `dismissed` filter. Terminology sweep: alert-rule type labels now read "Allowed" instead of "Accepted" (values unchanged). Dead v2 components removed (`AlertsTable`, `AlertFilters`, `RecentAlerts`, `NetworkCards`, `SystemInfo`). All 21 routes browser-verified (10 redirects + 11 pages, no console errors).
...
- **Frontend**: alert assignment silently never assigned — both `useAlertMutations().assign` and `AssignAlertDropdown` sent `{assigned_to_user_id}` to `PATCH /api/alerts/{id}/assign`, but the backend contract is `AlertAssignRequest.user_id`, so Pydantic dropped the unknown key and treated every request as an unassign. Both callers now send `{user_id}`.
- **Backend**: `severity_counts` in `GET /api/alerts/` changed with the page size — the host-cache loop in `list_alerts` used `for ip in unique_ips`, shadowing the `ip` query parameter, so the subsequent `_count_by_severity` call silently filtered on the last IP of the current page. Loop variable renamed; regression test asserts counts are independent of `limit`. (redesign per `docs/redesign/ui-v3-plan.md`, ADRs 0001–0007). The sidebar is restructured into the task-based areas Overview / Monitor (Dashboard, Alerts, Hosts, Scans) / Configuration (Networks, Scanners, Scan Templates, Alerting; operator+) / Administration (Users & Roles, Organization, System; admin only) — role visibility now uses a `hasRole` hierarchy (`lib/roles.ts`) instead of the old `adminOnly` flag, and retired v2 entries (NSE Scripts, Results, Alert Rules, GVM Library, Severity Rules, SSH Defaults, Hostname Cache, Roles, Trends) are no longer in the nav (pages remain reachable by URL until their phases replace them). New placeholder routes `/overview`, `/scan-templates`, `/alerting`, and `/admin/system` mark the future hubs. New shared infrastructure for the alert inbox: `lib/terminology.ts` (effect-based Allow/Mute wording from `docs/redesign/glossary.md`), `hooks/useKeyboardShortcuts.ts` (global shortcuts that never fire in inputs, dialogs, or menus, per the alert-state-action-matrix interaction rules), and `components/layout/SplitView.tsx` (list/detail inbox layout). The Quick Scan sidebar button is kept (parity-matrix decision). All new modules have vitest coverage.
...
- **Backend**: the nginx `/api/` proxy in the combined production image (`docker/nginx.conf`) now sets `client_max_body_size 64M`. It previously relied on nginx's 1 MiB default, so NSE result submissions from larger networks were rejected with `413 Request Entity Too Large` on `POST /api/nse/scanner/results` — the scanner's vulnerability phase raised on the 413 and every scheduled scan on the affected network was marked failed (observed in production: 83 consecutive failures on a 51-host network whose NSE payload exceeded 1 MiB, while a sibling network at ~750 KiB kept passing). The scan itself completed; only the result upload was dropped. Requires rebuilding/redeploying the combined image. `users.$userId.tsx` was renamed to `users_.$userId.tsx` so the TanStack route generator un-nests it from the `users` list route (which has no `<Outlet/>`); previously the checked-in `routeTree.gen.ts` was a stale flat tree that worked but would break `/admin/users/$userId` on any regeneration. URL is unchanged.
...
- **Backend**: upgrade pytest to 9.0.3 to fix insecure /tmp/pytest-of-{user} directory vulnerability (Dependabot #29)
- **Scanner**: upgrade pytest to 9.0.3 to fix insecure /tmp/pytest-of-{user} directory vulnerability (Dependabot #28)
- **Scanner**: upgrade Pygments to 2.20.0 to fix ReDoS vulnerability in AdlLexer (Dependabot #25)
...
- Translated all remaining German UI text to English in SSH alert defaults, SSH alert overrides, and networks filter
...
- Nuclei source filter in Alert Triage Center dropdown
...
- **Removed env vars** (silently ignored if still in `.env` — remove for cleanliness): `HOSTNAME_LOOKUP_ENABLED`, `HOSTNAME_LOOKUP_INTERVAL_MINUTES`
- **Surviving env vars** (still active, read by the scanner-facing budget endpoint): `HACKERTARGET_API_KEY`, `RAPIDDNS_ENABLED`, `RAPIDDNS_DAILY_LIMIT`
- **No kill switch**: there is no env var to re-enable the backend filler. Fallback is a Docker image rollback to the previous release + `alembic downgrade 015`
- **Migration 016 downgrade** drops `hostname_lookup_queue` and loses in-flight manual refresh requests. Drain the queue (wait one scanner poll ~10 s with no new clicks) before downgrading
- **Deploy sequence**: upgrade backend first, then all scanners together in one maintenance window. Mixed-version scanner clusters are not supported — old scanners cannot reach the new `/api/scanner/hostname-lookup-jobs` endpoint
- **Cold start**: the removed hourly filler used to enrich hosts passively. Post-upgrade, networks that don't run a scan won't get hostname enrichment until they do. Trigger a scan manually on infrequent networks, or use the per-host Refresh button

### Added

- **Hostname lookup cache** (migrations 014–016): reverse-IP vhost discovery for SNI-aware nuclei scans on shared-hosting targets. Two sources in priority chain: HackerTarget `/reverseiplookup/` (50/day anon, 100/day with `HACKERTARGET_API_KEY`, HTTP 429 → pin budget) and RapidDNS `/sameip/` HTML fallback (100/day default, Cloudflare-challenge → pin). Cache tables `hostname_lookup_cache` (IP → vhost list, TTL by status: 30d success / 7d no_results / 3d failed) and `hostname_lookup_budget` (per-source/day counter). Queue table `hostname_lookup_queue` (migration 016) backs on-demand manual lookups from the UI. Scanner owns all external API calls; backend stores cache + budget + queue and exposes scanner-facing endpoints: `GET /api/scanner/hostname-budget`, `POST /api/scanner/hostname-results`, `GET /api/scanner/hostname-lookup-jobs`, `POST /api/scanner/hostname-lookup-jobs/{id}/complete`. Export/import via `GET/POST /api/admin/hostname-lookup/export|import`
- **Nuclei SNI fan-out** (migration 015): per-network `nuclei_sni_enabled` flag expands nuclei targets from `IP:PORT` to `https://vhost:PORT` per cached vhost. Scanner queries `GET /api/scanner/hostnames` between port_scan and nuclei phases. `build_targets()` picks scheme via TLS port list + nmap `service_guess`, caps at 50 vhosts/IP, falls back to `IP:PORT` on cache miss. NetworkForm checkbox in the Nuclei section
- **Hostname cache admin UI** (`/admin/hostname-lookup`): new sidebar entry "Hostname Cache". Stat cards: cache coverage %, total vhosts, per-source daily budgets (auto-poll 15 s), pending queue count. Filterable entries table with status badges (success/no_results/failed), click-to-expand vhost chips, per-row Edit/Refresh/Delete actions. JSON Export/Import (`skip`/`overwrite` strategy). Manual-edited rows get amber accent
- **Hostname cache manual edit**: `PUT /api/admin/hostname-lookup/entries/{ip}` stores `source='manual'` with 8-week TTL (`MANUAL_EDIT_TTL_DAYS=56`) so the scanner skips the row until it expires. `DELETE /entries/{ip}` drops a row. Modal textarea (one hostname per line) on the admin page. Per-IP Refresh button enqueues on-demand lookup for the scanner to pick up (~10 s)
- **Hosts table vhost chip**: `/hosts` table hostname column falls back to `cached_display_hostname` when `host.hostname` is null. Compact `+N` chip on rows with cached vhosts — hover shows Radix tooltip with full vhost list (lazy-fetched on first open). Backend: `cached_hostname_count` + `cached_display_hostname` fields on `HostResponse`, populated via batch JOIN
- **Host detail Known Hostnames panel**: collapsible `HostKnownHostnames` card between HostComment and tabs. Shows count + source badge + "cached DATE", chip grid on expand, amber "MANUAL" badge for hand-edited rows. Always-visible Refresh button enqueues a manual lookup; results appear after ~10 s scanner poll. Renders even when no cache row exists (shows "click Refresh" hint)
- **Scanner enrichment refactor**: all 5 enrichment functions in `hostname_enrichment.py` return `dict[str, list[str]]` (HackerTarget now keeps the full vhost list instead of `domains[0]`). New `enrich_hostnames_rapiddns` slot as HT fallback. Budget-aware: pre-flights `GET /hostname-budget`, skips exhausted sources. Posts vhost-list outcomes (HT/RapidDNS/crt.sh) to `POST /hostname-results`; display-name sources (ssl-cert/PTR/ip-api) never reach the cache. Queue poller (`process_hostname_lookup_queue`) wired into `main.py` poll loop, drains on-demand refresh jobs alongside scan-job + host-discovery polling. Skipped on GVM-only containers
- **Per-network SSH probe toggle** (migration 013): `networks.ssh_probe_enabled` (default on) opts out networks from automatic `ssh-audit` + `nmap ssh-auth-methods` after port scan. Frontend checkbox in Schedule & Alerts tab

### Removed

- **Backend hostname cache filler**: `hostname_lookup_filler.py` + APScheduler job deleted. The scanner owns all external hostname API egress now; enrichment happens during host discovery + via on-demand queue polling
- **Backend source classes**: `HackerTargetSource`, `RapidDnsSource`, `HostnameLookupSource` Protocol, `lookup_with_cache`, and all supporting constants removed from `app.services.hostname_lookup`. Same classes live scanner-side in `scanner/src/hostname_sources.py`
- **`POST /api/admin/hostname-lookup/run-filler`** endpoint (returns 404)
- **Settings**: `hostname_lookup_enabled`, `hostname_lookup_interval_minutes` removed from `app.core.config`

### Changed

- **Network form dialog**: split into 3 tabs — General / Phases / Schedule & Alerts — eliminates vertical scroll. Validation errors on inactive tabs auto-focus their tab. Greenbone edge case: flipping scanner_type to Greenbone on Phases tab falls back to General
- **Nuclei scanner**: `-stats` progress now streams live to scan logs instead of being swallowed by `subprocess.run(capture_output=True)`. Switched to `Popen(stdout=PIPE, stderr=STDOUT, bufsize=1)` with `ProcessTimeoutWatcher`
- **Network form**: edit-mode submit button → "Save" (was "Update")

### Fixed

- **Nuclei scanner**: nuclei phase silently skipped on networks with stored `phases` column. `process_job` used `job.phases or _build_legacy_phases(job)` and only `_build_legacy_phases` appended nuclei. New `_ensure_nuclei_phase(phases, job)` helper runs for both legacy + pre-built lists
- **Scanner dev image**: `Dockerfile.dev` missing nuclei binary. Mirrored prod multi-stage copy, pinned `NUCLEI_VERSION=v3.7.1`
- **Nuclei templates**: build-time `-duc` flag short-circuited the template update itself. Removed from update step; runtime `-disable-update-check` stays
- **Network form**: Nuclei "Minimum severity" default `""` silently failed zod validation. Preprocess empty → `undefined`
